...
class Webdogs_Maintenance_Notification
{
    ...
    public static function init() {
        ...
        add_action( 'wds_after_validate', array( __CLASS__, 'report_schedule_changes' ) );
        add_action( 'wds_test_maintenance_notification', array( __CLASS__, 'send_test_maintenance_notification' ), 10, 1 );
        add_action( 'wds_scheduled_notification',        array( __CLASS__, 'send_maintenance_notification' ) );

        Self::$test  = ( isset( $_GET['wds_send_maintenance_notification'] ) && "test"===$_GET['wds_send_maintenance_notification'] );
        Self::$force = ( isset( $_GET['force_send'] )                        &&     ""!==$_GET['force_send'] );
        ...
        if( ( defined('DOING_CRON') && DOING_CRON ) || Self::$test ) {
            ...
            if ( ! function_exists( 'wds_domain_exculded' ) )      include_once WEBDOGS_SUPPORT_DIR . '/maintenance-support/webdogs-common.php';
            if ( ! function_exists( 'wds_base_strings' ) )         include_once WEBDOGS_SUPPORT_DIR . '/options-framework/options.php';
