...
if ( ! class_exists( 'Options_Framework_Plugin_Activation' ) ) {
	...
	class Options_Framework_Plugin_Activation {
		...
		/**
		 * Holds arrays of themes details.
		 *
		 * @since 1.0.0
		 *
		 * @since 2.5.0 the array has the theme slug as an associative key.
		 *
		 * @var array
		 */
		public $themes = array();
		...
		/**
		 * Whether any plugins have the 'force_deletion' setting set to true.
		 *
		 * @since 2.5.0
		 *
		 * @var bool
		 */
		protected $has_forced_deletion = false;
		 ...
		 * @var bool
		 */
		protected $has_theme_deletion = false;
		 ...
		 * @var string
		 */
		public $capability = 'manage_support';
		...
		public function init() {
			...
			&& (   empty( $this->themes  ) || ! is_array( $this->themes  ) ) ) {
				return;
			}
			...
			if ( true !== $this->is_optionsframework_complete() ) {
				...
				// Hide force deactivated plugins
		...
		public function hide_plugins() {
			...
			foreach ( $this->plugins as $slug => $plugin ) {
				if ( $this->is_plugin_active( $slug ) && false === $plugin['force_deactivation'] ) continue;
				if ( true === $plugin['force_deactivation'] && $this->is_plugin_installed( $slug ) ) { //if ( true === $plugin['force_deletion'] 
					if( in_array( $plugin['file_path'], array_keys( $plugins ) ) ) { 
				 		unset($wp_list_table->items[ $plugin['file_path'] ]); 
				 	} 
				}
			}
		 	// return $plugins; 
		...
		/**
		 * Add individual theme to our collection of themes.
		 *
		 * If the required keys are not set or the theme has already
		 * been registered, the theme is not added.
		 *
		 * @since 2.0.0
		 *
		 * @param array|null $theme Array of theme arguments or null if invalid argument.
		...
		public function enqueue( $theme ) {
			...
			if ( empty( $theme['slug'] ) || ! is_string( $theme['slug'] ) || isset( $this->themes[ $theme['slug'] ] ) ) {
				return;
			}
			...
			$defaults = array(
				...
				'slug'               => '',      // String
				'active'             => true,    // Boolean
				'force_deletion'     => false,   // Boolean
			);

			// Prepare the received data.
			$theme = wp_parse_args( $theme, $defaults );
			...
			// $this->sort_order[ $theme['slug'] ] = $theme['name'];

			// Should we add the force deletion hook ?
			if ( true === $theme['force_deletion'] ) {
				$this->has_theme_deletion = true;
			}
		}
		...
		public function register( $plugin ) {
			...
			// Should we add the force deletion hook ?
			if ( true === $plugin['force_deletion'] ) {
				$this->has_forced_deletion = true;
			}
		}
		...
		public function can_plugin_update( $slug ) {
			if ( true === $this->plugins[ $slug ]['force_deactivation'] || true === $this->plugins[ $slug ]['force_deletion'] ) {
				return false;
			}
		...
		/**
		 * Forces plugin deletion if the parameter 'force_deletion'
		 * is set to true.
		 *
		 * This allows theme authors to specify certain plugins that must be
		 * deleted upon switching from the current theme to another.
		 *
		 * Please take special care when using this parameter as it has the
		 * potential to be harmful if not used correctly.
		 *
		 * @since 2.2.0
		 */
		public function force_deletion() {
			...
			foreach ( $this->plugins as $slug => $plugin ) {
				...
				if ( true === $plugin['force_deletion'] && $this->is_plugin_installed( $slug ) ) {
					$delete[] = $plugin['file_path'];
					$deleted_names[] = $plugin['name'];
				}
			}
		...
		public function theme_deletion() {
			...
			if ( sizeof( $this->themes ) && $can_delete ) {
	    		foreach ($this->themes as $slug => $theme  ) {
	    			if ( true === $theme['active'] || false === $theme['force_deletion'] ) { continue; }
			        $delete_result = delete_theme( $slug );
			        if( is_wp_error( $delete_result ) ) {
			        	$error[] = $delete_result->get_error_message();
			        } else {
				        $deleted_names[] = $theme['name'];
			        }
			    }
...
if ( ! function_exists( 'Options_Framework_Register_Plugins' ) ) {
	...
	function Options_Framework_Register_Plugins( $plugins, $themes, $config = array() ) {
		...
		foreach ( $themes as $theme ) {
			call_user_func( array( $instance, 'enqueue' ), $theme );
		}
	...
	class Options_Framework_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( $plugins[ $this->view_context ] as $slug => $plugin ) {
				// No deactivate
				if( true === $plugin['force_deactivation'] || true === $plugin['force_deletion'] ) continue;
